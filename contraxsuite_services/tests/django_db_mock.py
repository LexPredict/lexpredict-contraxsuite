"""
    Copyright (C) 2017, ContraxSuite, LLC

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as
    published by the Free Software Foundation, either version 3 of the
    License, or (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <http://www.gnu.org/licenses/>.

    You can also be released from the requirements of the license by purchasing
    a commercial license from ContraxSuite, LLC. Buying such a license is
    mandatory as soon as you develop commercial activities involving ContraxSuite
    software without disclosing the source code of your own applications.  These
    activities include: offering paid services to customers as an ASP or "cloud"
    provider, processing documents on the fly in a web application,
    or shipping ContraxSuite within a closed source product.
"""
# -*- coding: utf-8 -*-


__author__ = "ContraxSuite, LLC; LexPredict, LLC"
__copyright__ = "Copyright 2015-2020, ContraxSuite, LLC"
__license__ = "https://github.com/LexPredict/lexpredict-contraxsuite/blob/1.5.0/LICENSE"
__version__ = "1.5.0"
__maintainer__ = "LexPredict, LLC"
__email__ = "support@contraxsuite.com"

from typing import List, Any, Optional, Callable, Dict


class ValuesList:
    def __init__(self, objects: List[Any]):
        self.index = 0
        self.objects = objects

    def __getitem__(self, key):
        return self.objects[key]

    def __iter__(self):
        return self

    def __next__(self):
        if self.index >= len(self.objects):
            raise StopIteration
        o = self.objects[self.index]
        self.index += 1
        return o

    def distinct(self):
        objects = list(set(self.objects))
        return ValuesList(objects)


class MockQuerySet:
    def __init__(self, objects: Optional[List[Any]] = None):
        self.objects = objects
        self.index = 0
        self.custom_filter: Optional[Callable[[List[Any], Dict[str, Any]], List[Any]]] = None

    def filter(self, *args, **kwargs):
        if self.custom_filter:
            filtered = self.custom_filter(self.objects, kwargs)
            return MockQuerySet(filtered)
        return self

    def distinct(self, *args, **kwargs):
        return self

    def __getitem__(self, key):
        return self.objects[key]

    def __iter__(self):
        return self

    def __next__(self):
        if self.index >= len(self.objects):
            self.index = 0
            raise StopIteration
        o = self.objects[self.index]
        self.index += 1
        return o

    def exists(self, *args, **kwargs):
        return len(self.objects) > 0

    def values_list(self, *args, **kwargs):
        items = []
        for doc in self.objects:
            item = ()
            if kwargs.get('flat', False):
                items.append(getattr(doc, args[0]))
            else:
                for arg in args:
                    val = getattr(doc, arg)
                    item += (val,)
                items.append(item)
        return ValuesList(items)


class MockObjectManager:
    def __init__(self):
        self.objects = MockQuerySet()
