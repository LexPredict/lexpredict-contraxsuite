...
class LegalTermUsageListView(JqPaginatedListView):
    model = LegalTermUsage
...
class TopLegalTermUsageListView(JqPaginatedListView):
    model = LegalTermUsage
    template_name = "extract/top_legal_term_usage_list.html"
    limit_reviewers_qs_by_field = 'text_unit__document'

    def get_json_data(self, **kwargs):
        data = list(self.get_queryset())
        if "document_pk" in self.request.GET:
            term_list_view = LegalTermUsageListView(request=self.request)
            term_data = term_list_view.get_json_data()['data']
        for item in data:
            item['url'] = reverse('extract:legal-term-usage-list') + \
...
class TypeaheadLegalTermTerm(TypeaheadView):
    model = LegalTerm
    search_field = 'term'
    ...
    def qs_to_values(self, qs):
        results = []
        for t in qs:
            if t.legaltermusage_set.count() > 0:
                results.append({"value": t.term})
                results.append({"value": '-%s' % t.term})
        return results
