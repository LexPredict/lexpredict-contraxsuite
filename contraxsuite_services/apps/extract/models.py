...
class LegalTerm(models.Model):
    ...
    def __str__(self):
        return "LegalTerm (term={0}, source={1})" \
            .format(self.term, self.source)


class LegalTermUsage(models.Model):
    """
    Legal term/dictionary usage
    """
    text_unit = models.ForeignKey(TextUnit, db_index=True)
    term = models.ForeignKey(LegalTerm, db_index=True)
    ...
    def __str__(self):
        return "LegalTermUsage (term={0}, text_unit={1}, count={2})" \
...
class GeoAlias(models.Model):
    ...
    alias = models.CharField(max_length=10, db_index=True)
