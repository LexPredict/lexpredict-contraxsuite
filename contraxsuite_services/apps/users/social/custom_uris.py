...
def get_callback_url(adapter: OAuth2Adapter, request, app):
    callback_url = settings.SOCIAL_CALLBACK_URL_TEMPLATE.format(adapter.provider_id)
    return build_absolute_uri(request, callback_url)

    # callback_url = reverse(provider.provider_id + "_callback")
    # protocol = getattr(settings, 'OAUTH_CALLBACK_PROTOCOL') or provider.redirect_uri_protocol or 'https'
    # try:
    #     url = build_absolute_uri(request, callback_url, protocol)
    #     if protocol == 'https' and url.startswith('http:'):
    #         url = 'https' + url[len('http'):]
    #     logger.info(f'OAuth callback URL is "{url}", protocol is "{protocol}"')
    #     return url
