...
class TaskVisibility:
    @classmethod
    def get_excluded_from_tracking(cls) -> Set[str]:
        ...
        extra_excluded = {n for n in (EXTRA_EXCLUDED_FROM_TRACKING.val() or '').strip().split(',') if n}
        extra_allowed = {n for n in (EXTRA_ALLOWED_FOR_TRACKING.val() or '').strip().split(',') if n}
